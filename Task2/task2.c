...
uint16_t  ADC1ConvertedValue1   = 0;
uint16_t  ADC1ConvertedVoltage1 = 0;
uint16_t  ADC1ConvertedValue2   = 0;
uint16_t  ADC1ConvertedVoltage2 = 0;
...
int main(void) 
{
  ...
  ADC1->SQR1 |= (7 << 6) | (6<<12) | (1<<0);

  ADC1->SMPR1 |= (5 << 21) | (2<< 18);                   // SMP7[2:0] = 0b011; sampling time 7.5 ADC clock cycles
 ...
 while (1)
  {
    ...
    ADC1ConvertedValue1 = ADC1->DR;          // ADC1ConvertedValue = RDATA[15:0]
    ADC1ConvertedVoltage1 = (ADC1ConvertedValue1 *3000)/4095; // Compute the voltage
		while(!(ADC1->ISR & (1 << 2)));
		ADC1ConvertedValue2 = ADC1->DR;
		ADC1ConvertedVoltage2 = (ADC1ConvertedValue2 *3000)/4095;
		temperature= ADC1ConvertedVoltage2/10;
  }
